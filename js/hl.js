/* jQuery CodeLighter version 0.1
 * author: Pablo Cazorla - pablo.cazorla@mercadolibre.com
 */
;(function(){
...
(function($){
    $.fn.highlight = function(customSettings) {
        return this.each(function(){
        	 var setting = {
				...
				showTools : true,
				showBtnNum : true,
				showBtnFormatted : true,
				
				labels : {
					showNum : 'Show numlines',
					hideNum : 'Hide numlines',
					Formatted : 'Formated',
					Unformatted : 'Unformated'
				}
			}
			$.extend(setting,customSettings);
            
            var $p = $(this);
            
            var cl = $p.attr('class'); 
            if(cl !== 'none' && cl !== '' && cl !== undefined){
	            var stOrig =  $p.html();
	            stOrig = $$hl.set(stOrig).saveHtmlTags().result();
	                   
	              
	                       
	     		var st = $$hl.paint(stOrig,cl,setting.customStyle);
	     		if(setting.showNum){
	     			st = '<span class="num"></span>' + st.replace(/\n/g,'$&<span class="num"></span>');
	       		}
	       		       		
	       		var $contOrig = $('<div class="hlOrig">').html(stOrig)
	            var $contForm = $('<div class="hlForm">').html(st);
	       		
	       		
	       		
	       		if(setting.showNum){
	       			$contForm.addClass('showNum');
		       		var $nums = $contForm.find('.num');
		       		var n = '';
		       		var $nums_length = $nums.length -1;
		       		for(ie=0;ie<$nums_length;ie++){
		       			n = (ie+1);
		       			$nums.eq(ie).html(n);
		       		}
	       		}
	       		...
	       		if(setting.showNum){
	       			var labelNum = setting.labels.hideNum;
	       		}else{
	       			var labelNum = setting.labels.showNum;
	       		}
	       		
	       		if(setting.showFormatted){
	       			$contOrig.hide();
	       			var labelShow = setting.labels.Unformatted;
	       		}else{
	       			$contForm.hide();
	       			var labelShow = setting.labels.Formatted;
	       		}
	       		
	       		/* TOOLS */
	       		if(setting.showTools){
		       		var $tools = $('<div class="hlTools">').hide();
		       		
		       		
		       		       				
		       		if(setting.showBtnNum){
			       		var $btn_num = $('<a href="">'+labelNum+'</a>');
			       		$btn_num.click(function(ev){
			       			ev.preventDefault();
			       			if(setting.showNum){
			       				$contForm.removeClass('showNum');
			       				setting.showNum = false;
			       				$btn_num.html(setting.labels.showNum);
			       			}else{
			       				$contForm.addClass('showNum');
			       				setting.showNum = true;
			       				$btn_num.html(setting.labels.hideNum);
			       			}       			
			       		});
			       		$tools.append($btn_num);
					}
					
					if(setting.showBtnFormatted){
			       		var $btn_show = $('<a href="">'+labelShow+'</a>');
			       		$btn_show.click(function(ev){
			       			ev.preventDefault();
			       			if(setting.showFormatted){
			       				$contForm.hide();
			       				$contOrig.show();
			       				setting.showFormatted = false;
			       				$btn_show.html(setting.labels.Formatted);
			       				if(setting.showBtnNum){
			       					$btn_num.hide();
			       				}
			       			}else{
			       				$contForm.show();
			       				$contOrig.hide();
			       				setting.showFormatted = true;
			       				$btn_show.html(setting.labels.Unformatted);
			       				if(setting.showBtnNum){
			       					$btn_num.show();
			       				}
			       			}       			
			       		});
			       		$tools.append($btn_show);
					}
	       		...
	       		$p.append($contForm).append($contOrig);
	        }
...
$('pre').highlight({showTools : false,showNum : false});
